const mailjet = require('node-mailjet').apiConnect(
  'bd27d1f8c3533a3f812cb4faf2b5e9b1', // MJ_APIKEY_PUBLIC
  'c1355ff54acd472548d52ac1379cc7b0'  // MJ_APIKEY_PRIVATE
);
...
// Función para enviar campaña usando la librería oficial
async function sendCampaign() {
  try {
    ...
    const messages = TEST_LEADS.map(lead => ({
      From: {
        Email: 'no-reply@ritterfinder.com',
        Name: 'RitterFinder'
      },
      To: [
        {
      ...
      Subject: personalizeContent('🚀 Prueba de Campaña - RitterFinder Energías Renovables', lead),
      TextPart: personalizeContent(TEXT_TEMPLATE, lead),
      HTMLPart: personalizeContent(HTML_TEMPLATE, lead)
    }));

    console.log('\n📤 Enviando a Mailjet API usando librería oficial...');
    ...
    console.log(`   - Mensajes procesados: ${result.body.Messages?.length || 0}`);
    
    if (result.body.Messages) {
      result.body.Messages.forEach((msg, index) => {
...
function checkConfig() {
  ...
  console.log(`   - API Secret: ✅ Configurada`);
  console.log(`   - Librería: node-mailjet`);
...
// Ejecutar script
async function main() {
  console.log('📧 Test Script - Mailjet Campaign Service (Librería Oficial)');
  console.log('============================================================\n');
